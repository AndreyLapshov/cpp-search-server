...
explicit SearchServer(const StringContainer& stop_words) { 
        for (const auto& stop_word : stop_words) { 
            if (ContainsInvalidCharacters(stop_word)) { 
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (ContainsInvalidCharacters(document)) {
...
private:
    ...
    bool ContainsInvalidCharacters(const string& text) const {
...
static int ComputeAverageRating(const vector<int>& ratings) {
    ...
    double ComputeWordInverseDocumentFreq(const string& word) const {
        return log(GetDocumentCount() * 1.0f / word_to_document_freqs_.at(word).size());
    }
    template <typename DocumentPredicate>
